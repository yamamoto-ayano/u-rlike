...
export function SwipeCard({ id, title, url, description, imageUrl, onSwipe }: SwipeCardProps) {
  ...
  const updatePosition = () => {
    if (cardRef.current) {
      ...
      if (isDragging && (absX > 100 || absY > 100)) {
        if (absX > absY) {
          if (offsetX.current > 0) {
            cardRef.current.style.boxShadow = `0 0 20px rgba(255, 75, 75, ${Math.min(Math.abs(offsetX.current) / 200, 0.8)})`;
          }
          else {
            cardRef.current.style.boxShadow = `0 0 20px rgba(156, 163, 175, ${Math.min(Math.abs(offsetX.current) / 200, 0.8)})`;
          }
        } else if (offsetY.current < 0) {
          cardRef.current.style.boxShadow = `0 0 20px rgba(75, 123, 255, ${Math.min(Math.abs(offsetY.current) / 200, 0.8)})`;
        } else {
      ...
      cardRef.current.style.transform = `translate(${offsetX.current}px, ${offsetY.current}px) rotate(${offsetX.current * 0.15}deg)`;
    }
    ...
    if ((absX > 100 || absY > 100) && !(absY > absX && offsetY.current > 0)) {
      ...
      if (direction) {
        ...
        cardRef.current?.style.setProperty(
          "transform",
          `translate(${direction === "right" ? window.innerWidth : direction === "left" ? -window.innerWidth : 0}px, ${direction === "up" ? -window.innerHeight : 0}px)`
        );

        setTimeout(() => {
          cardRef.current?.style.setProperty("display", "none");
          cardRef.current?.style.setProperty("transform", "translate(0, 0)");
          cardRef.current?.style.setProperty("box-shadow", "0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05)");
        }, 200);
  ...
  return (
    <div
      ref={cardRef}
      className={cn(
        "swipe-card rounded-lg overflow-hidden shadow-lg bg-white transition-all duration-300 w-full select-none",
    ...
    >
      ...
      <div className="p-4">
        ...
        <p className="text-sm mt-4">{url}</p>
      </div>
    </div>
  );
}
